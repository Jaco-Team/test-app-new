...
export default function FormOrder({ cityName }) {
  ...
  const [openModalAddr, getOrderList, userInfo, getUserInfo] = useProfileStore(
    (state) => [
      state.openModalAddr,
      state.getOrderList,
      state.userInfo,
  ...
  const isMobileOrder = useMediaQuery(
    `screen and (max-width: ${BREAKPOINTS.mobileMax}px)`
  );

  const [thisCityList, thisCity, thisCityRu, setThisCityRu] = useCitiesStore(
    (state) => [
      state.thisCityList,
      state.thisCity,
      state.thisCityRu,
  ...
  const [
    ...
    state.getInfoPromo,
    state.checkPromo,
    state.setActiveMenuCart,
    state.pointList,
    state.getMySavedAddr,
    state.createOrder,
    ...
    state.sdacha,
    state.setSdacha,
    state.check_need_dops,
    state.cart_is,
    state.free_drive,
    state.setConfirmForm,
    state.getNewPriceItems,
    state.setMailForm,
    state.checkNewOrder,
    state.setDopsForm,
    state.openMailForm,
  ...
  const [continueCheckoutAfterEmail, setContinueCheckoutAfterEmail] =
    useCartStore((state) => [
      state.continueCheckoutAfterEmail,
      state.setContinueCheckoutAfterEmail,
    ]);
  ...
  useEffect(() => {
    if (addrList && addrList?.length > 0 && orderAddr) {
      let findAddr = addrList.find(
        (addr) => parseInt(addr?.id) == parseInt(orderAddr?.id)
      );
  ...
  useEffect(() => {
    if (
      dayjs(new Date()).locale('ru').format('YYYY-MM-DD') > dateTimeOrder?.date
    ) {
  ...
  useEffect(() => {
    ...
    setContinueCheckoutAfterEmail(false);
    create_order();
  }, [continueCheckoutAfterEmail, openMailForm, userInfo?.mail]);
  ...
  const openMenu = (event, nameList) => {
    ...
    if (nameList === 'point') {
      ...
      if (isMobileOrder) {
        ...
        const points = pointList.filter(
          (point) => point.name_ru === thisCityRu
        );
        setList(points);
        setAnchorEl(event.currentTarget);
      }
    ...
    if (nameList === 'addr') {
      ...
      if (isMobileOrder) {
        ...
        const newAddr = addrList?.find(
          (item) => item.name === 'Добавить новый адрес'
        );
        if (!newAddr) {
          addrList?.push({
            name: 'Добавить новый адрес',
            id: addrList.length + 1,
          });
        }
    ...
    if (nameList === 'time') {
      ...
      if (point_id) {
        ...
        setActiveModalAlert(
          true,
          'Необходимо выбрать ' +
            (typeOrder == 'pic' ? 'Кафе' : 'Адрес доставки'),
          false
        );
      }
    }
    ...
    if (nameList === 'pay') {
      ...
      if (typeOrder == 'pic') {
        if (
          parseInt(userInfo?.id) == 103162 ||
          parseInt(userInfo?.id) == 17622
        ) {
  ...
  useEffect(() => {
    if (
      typePay?.id == 'online' &&
      [1, 2, 3].includes(parseInt(orderAddr?.point_id))
    ) {
      if (
        dayjs(new Date()).locale('ru').format('YYYY-MM-DD') >= '2024-12-14' &&
        dayjs(new Date()).locale('ru').format('YYYY-MM-DD') <= '2024-12-31'
      ) {
        setActiveModalAlert(
          true,
          'По техническим причинам оплата банковской картой временно недоступна. Оплату можно будет осуществить только наличными. Будем благодарны, если вы подготовите сумму без сдачи.',
          false
        );
      }
    }
  ...
  let text_dev =
    'Уважаемые клиенты, на сайте указано приблизительное время готовности заказа и доставки. В зависимости от ситуации на дорогах время доставки может быть увеличено. Благодарим за понимание!';

  if ([4, 5, 7, 8].includes(parseInt(orderAddr?.point_id))) {
    text_dev =
      'Уважаемые клиенты! Из-за перебоев в работе мобильного интернета возможны задержки в доставке заказов. Благодарим за понимание.';
  }
        ...
        <div className="CartMobileContainer">
          ...
          <Stack
            ...
            style={{
              ...
              marginBottom: '6.8376068376068vw',
            }}
          >
            <MySwitchMobile
              onClick={(event) => setTypeOrder(event.target.checked)}
              checked={typeOrder == 'pic' ? true : false}
            />
          </Stack>

          <Button
            className="CartChoose"
            onClick={(event) => openMenu(event, 'city')}
            endIcon={<ArrowDownCartMobile />}
          >
            <div>
          ...
          {typeOrder == 'pic' ? (
            <Button
              className="CartChoose"
              onClick={(event) => openMenu(event, 'point')}
              endIcon={<ArrowDownCartMobile />}
            >
              <div>
                <Typography component="span">
                ...
                {orderAddr?.addr_name ? (
                  <Typography
                    ...
                    style={{
                      ...
                      maxWidth: '63.247863247863vw',
                    }}
                    className={
                      orderAddr?.addr_name?.length > 15 ? 'shadowSpan' : null
                    }
                  >
                    {orderAddr?.addr_name}
                  </Typography>
                  ...
                  <Typography
                    component="span"
                    className={
                      orderAddr?.name?.length > 29 ? 'shadowSpan' : null
                    }
                  >
                    {orderAddr?.name}
                  </Typography>
          ...
          >
            <div>
              ...
              <Typography component="span">
                {dateTimeOrder
                  ? dateTimeOrder?.name === 'В ближайшее время'
                    ? dateTimeOrder?.name
                    : dateTimeOrder.text + ' к ' + dateTimeOrder.name
                  : type_pred[0].name}
              </Typography>
            </div>
          </Button>
          ...
          <Button
            ...
            style={{
              marginBottom:
                typeOrder == 'pic' ? '6.8376068376068vw' : '3.4188034188034vw',
            }}
            ...
            >
              <div>
                ...
                {message ? (
                  <Typography
                    component="span"
                    className={message.length > 20 ? 'comment' : null}
                    style={{
                      height:
                        message.length > 20
                          ? '13.675213675214vw'
          ...
          <Accordion
            className="CartItemDopText_accordion"
            style={{
              marginBottom: itemsOffDops.length ? null : '3.4188034188034vw',
            }}
          >
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Typography>Об аллергенах</Typography>
            </AccordionSummary>
            <AccordionDetails>{dopText_2}</AccordionDetails>
          </Accordion>
          ...
          <div
            className="CartTotal"
            style={{ marginTop: typeOrder == 'pic' ? '5.1282051282051vw' : 0 }}
          >
            <span>
            ...
            <div>
              ...
              >
                {new Intl.NumberFormat('ru-RU').format(
                  parseInt(allPriceWithoutPromo_new) + parseInt(NewSummDiv)
                )}{' '}
                ₽
              </span>
            </div>
          ...
          <div className="CartPromo">
            ...
            {checkPromo ? (
              checkPromo.st && itemsOffDops.length ? (
                <div>
                  {new Intl.NumberFormat('ru-RU').format(
                    parseInt(allPrice) + parseInt(NewSummDiv)
                  )}{' '}
                  ₽
                </div>
              ) : null
          ...
          {typeOrder == 'pic' ? (
            <span className="basketDopText">
              {parseInt(hours) >= 21
                ? 'Заказы на самовывоз выдаются до 22:00'
                : ''}
            </span>
          ) : (
          ...
          <div className="modalBasketContainer">
            ...
            <Stack
              ...
              style={{
                ...
                marginBottom: '2.8880866425993vw',
              }}
            >
              <MySwitchPC
                onClick={(event) => setTypeOrder(event.target.checked)}
                ...
                <div>
                  ...
                  {orderAddr?.addr_name ? (
                    <Typography
                      ...
                      className={
                        orderAddr?.addr_name?.length > 19 ? 'shadowSpan' : null
                      }
                    >
                      {orderAddr?.addr_name}
                    </Typography>
                    ...
                    <Typography
                      component="span"
                      className={
                        orderAddr?.name?.length > 31 ? 'shadowSpan' : null
                      }
                    >
                      {orderAddr?.name}
                    </Typography>
