...
export default memo(function CartItemMobile({ item, count, last }) {
  ...
  const [minus, plus, promoInfo, allItems] = useCartStore((state) => [
    state.minus,
    state.plus,
    state.promoInfo,
    state.allItems,
  ...
  return (
    <div
      ...
      style={{
        borderBottom: last
          ? 'none'
          : '0.17094017094017vw solid rgba(0, 0, 0, 0.1)',
      }}
    >
      ...
      <div className="CartItemsButton">
        <button
          className="minus"
          style={{
            backgroundColor: item?.disabled ? '#fff' : 'rgba(0, 0, 0, 0.07)',
        ...
        <button
          ...
          style={{
            backgroundColor:
              item?.disabled || count > 98 ? '#fff' : 'rgba(0, 0, 0, 0.07)',
            color:
