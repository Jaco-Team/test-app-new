...
export default function MailForm({ cityName }) {
  ...
  const isMobileMailForm = useMediaQuery(
    `screen and (max-width: ${BREAKPOINTS.mobileMax}px)`
  );
  const [openMailForm, setMailForm] = useCartStore((state) => [
    state.openMailForm,
    state.setMailForm,
  ]);
        ...
        >
          <div className="container">
            ...
            <span className="mailText">
              Электронная почта необходима для онлайн оплаты
            </span>
            <span className="mailText mailText_2">
              На электронную почту будет направлен чек об оплате
            </span>
