        ...
        <div>
          ...
          <div
            ...
            style={{
              cursor: 'pointer',
              justifyContent:
                parseInt(item.cat_id) == 4 ? 'space-evenly' : 'center',
            }}
            onClick={() => getItem('home', thisCity, item.id)}
          >
            ...
            {parseInt(item.cat_id) == 5 ||
            parseInt(item.cat_id) == 6 ||
            parseInt(item.cat_id) == 7 ||
            parseInt(item.cat_id) == 15 ? null : (
              <span
                className={`second_text ${hasDopSideOffset ? 'withSideOffset' : ''}`}
              >
                {parseInt(item.cat_id) == 14
                  ? item.size_pizza
                  : item.count_part}{' '}
                {parseInt(item.cat_id) == 14
                  ? 'см'
            ...
            <span
              className={`third_text ${hasDopSideOffset ? 'withSideOffset' : ''}`}
            >
              {new Intl.NumberFormat('ru-RU').format(item.weight)}{' '}
              {parseInt(item.id) == 17 || parseInt(item.id) == 237
                ? 'шт.'
                : parseInt(item.cat_id) == 6
                  ? 'л'
                  : 'г'}
