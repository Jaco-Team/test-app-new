...
export default function ModalItemPC() {
  const [
    ...
    state.openItemCard,
    state.closeItemModal,
    state.typeModal_dop,
    state.item_card,
    state.foodValue,
    state.navigate_dop,
    state.closeTypeModal_dop,
  ]);
  ...
  return (
    <Dialog
      onClose={(event, reason) =>
        reason === 'backdropClick' && typeModal_dop === 'start'
          ? closeItemModal()
          : navigate_dop('start')
      }
    ...
    >
      <DialogContent
        ...
        style={{
          background: typeModal_dop === 'start' ? '#FFFFFF' : '#E6E6E6',
        }}
      >
          ...
          <Grid container>
              ...
              <div className="ImgItem">
                ...
                <div className="badgecontainer">
                  ...
                  {item_card?.tags?.includes(14) === true ? (
                    <BadgeItem size={'big'} type={'hot'} view={'pc'} />
                  ) : (
                    false
                  )}
                </div>
            ...
            {typeModal_dop !== 'value' ? null : (
              <ClickAwayListener
                ...
                onClickAway={(event) => closeTypeModal_dop(event)}
              >
                    ...
                    <div className="List">
                      {links?.link_allergens ?? (
                        <div className="ValueTitle">
                          <Typography
                            variant="h5"
                            component="h2"
                            className="ItemTitleValue"
                          >
                            Полное описание состава блюд, калорийности и
                            возможных аллергенов можно{' '}
                            <Link
                              href={links?.link_allergens}
                              target="_blank"
                              style={{ color: '#DD1A32', cursor: 'pointer' }}
                            >
                      ...
                      {(item_card?.items ?? []).map((item, idx, arr) => (
                        <div
                          ...
                          className={
                            'ValueItem ' +
                            (idx === arr.length - 1 ? 'valueItemLastShort' : '')
                          }
                        >
            ...
            >
              <Grid className="SecondItem">
                ...
                <div className="dop">
                  <div className="dop_text">
                    ...
                    <span
                      className={
                        'third_text ' +
                        (parseInt(item_card?.count_part) == 1
                          ? 'dopThirdNoPad'
                          : 'dopThirdPad')
                      }
                    >
                  ...
                  <div
                    className="dop_icon"
                    style={{
                      cursor: typeModal_dop === 'start' ? 'pointer' : null,
                      visibility:
                ...
                <Typography
                  variant="h5"
                  component="span"
                  className="desk ItemDescStart"
                >
                  {item_card?.marc_desc_full.length > 0
                    ? item_card?.marc_desc_full
                    : item_card?.tmp_desc}
                </Typography>
                ...
                {count == 0 ? (
                  ...
                  <div className="containerBTN">
                    <div
                      style={{
                        backgroundColor:
                          typeModal_dop === 'start' ? '#ffff' : '#E6E6E6',
                      }}
                      onClick={
                        typeModal_dop === 'start'
                          ? null
