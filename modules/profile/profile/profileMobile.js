...
export default function ProfileMobile({ city, this_module }) {
  ...
  const [setActiveProfileModal, setUser, userInfo, getUserInfo, updateUser] =
    useProfileStore((state) => [
      state.setActiveProfileModal,
      state.setUser,
      state.userInfo,
      state.getUserInfo,
      state.updateUser,
  ...
  return (
    ...
    >
      ...
      <div className="profileData profileMain">
        ...
        <div
          className="dataItem profileMain" /*onClick={() => setActiveProfileModal(true, 'phone')}*/
        >
        ...
        <div className="dataItem profileMain">
          <span
            className="itemSwitch"
            style={{
              color:
                parseInt(userInfo?.spam) === 1
                  ? 'rgba(0, 0, 0, 0.80)'
                  : 'rgba(0, 0, 0, 0.40)',
            }}
          >
            Хочу получать рассылки с акциями, скидками и подарками
          </span>
          <MySwitch
            checked={parseInt(isSpam) == 1 ? true : false}
            onClick={(event) => {
              changeOtherData('spam', event.target.checked);
            }}
          />
        </div>
      </div>
