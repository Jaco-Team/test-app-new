...
export default function ProfilePC({ page, this_module, city }) {
  ...
  const [token, signOut] = useHeaderStoreNew((state) => [
    state?.token,
    state?.signOut,
  ]);
      ...
      <Grid item className="Profile mainContainer">
        ...
        <Grid item xs={12} className="check_data">
          <div>
            <div>
              ...
              <MySwitch
                checked={parseInt(isSpam) == 1 ? true : false}
                onClick={(event) => {
                  changeOtherData('spam', event.target.checked, userInfo);
                }}
              />
            </div>
        ...
        <Grid item xs={12} className="addr_table">
          <Table>
            <TableHead>
              <TableRow>
                <TableCell colSpan={3} className="headTable">
                  Мои адреса
                </TableCell>
                <TableCell colSpan={2} className="headAddTable">
                  <div
                    className="headAddTable__"
                    onClick={() => openModalAddr(0, city)}
                  >
                    <span>Добавить</span>
                  </div>
                </TableCell>
            ...
            <TableBody>
              {streets?.map((item, key) => (
                <TableRow key={key}>
                  ...
                  <TableCell
                    className={parseInt(item.is_main) == 1 ? 'ChooseAddr' : ''}
                  >
                    {parseInt(item.is_main) == 1 ? <div>Основной</div> : false}
                  </TableCell>
                  <TableCell className="ChangeAddr">
                    <span onClick={() => openModalAddr(item.id, item.city)}>
                      Изменить
                    </span>
                  </TableCell>
                  <TableCell>
                    <CloseIconMin onClick={() => delAddr(item.id, token)} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
