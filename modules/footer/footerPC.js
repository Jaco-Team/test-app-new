...
export default React.memo(function FooterPC({ cityName, active_page, links }) {
  ...
  const [badge_filter, tag_filter, text_filter] = useHomeStore((state) => [
    state.badge_filter,
    state.tag_filter,
    state.text_filter,
  ]);
  ...
  return (
    <>
      <div
        className={showArrow ? 'ArrowPC' : 'ArrowHidden'}
        onClick={scrollUp}
        style={{
          marginTop:
            active_page === 'sitemap' ||
            ...
            (active_page === 'home' &&
              ...
              ? '-4.3321299638989vw'
              : null,
          transform: active_page === 'contacts' ? 'translate(0, -50%)' : null,
        }}
      >
        <ArrowUp />
      </div>
      <footer
        className={`footerPC ${active_page === 'contacts' ? 'footerPC--contacts' : ''}`}
        style={footerStyle}
      >
