...
import { FormOrder_btn } from '../order-form-button/FormOrder_btn';
    ...
    <>
      <div className="modalFormOrder">
        ...
        <FormOrder_btn
          ...
          onClick={(e) =>
            openMenu(e, typeOrder === 'pickup' ? 'default' : 'address')
          }
          open={isOpenCat}
        />
        <FormOrder_btn
        ...
        <FormOrder_btn
          icon="card"
          placeholder={
            !online ? (typeOrder === 'pickup' ? '' : 'Способо оплаты') : ''
          }
        ...
        {typeOrder === 'pickup' ? null : (
          <FormOrder_btn
            icon="comment"
            placeholder={comment ? '' : 'Сообщение курьеру'}
        ...
        >
          ...
          <span>
            {new Intl.NumberFormat('ru-RU').format(
              allPrice
                ? parseInt(allPrice) + parseInt(summ)
                : parseInt(allPriceWithoutPromo_new) + parseInt(summ)
            )}{' '}
            ₽
          </span>
        </div>
