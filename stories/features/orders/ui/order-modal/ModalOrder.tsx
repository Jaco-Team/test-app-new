...
const OrderCartIcon = CartModalOrderIcon as React.ComponentType<
  Record<string, unknown>
>;
const OrderCookIcon = CookModalOrderIcon as React.ComponentType<
  Record<string, unknown>
>;
const OrderDeliveryIcon = DeliveryModalOrderIcon as React.ComponentType<
  Record<string, unknown>
>;
const OrderHomeIcon = HomeModalOrderIcon as React.ComponentType<
  Record<string, unknown>
>;
const OrderPicupIcon = PicupModalOrderIcon as React.ComponentType<
  Record<string, unknown>
>;
...
export const ModalOrder = ({
  ...
  if (parseInt(modalOrder?.order?.is_delete) == 1) {
    ...
    if (parseInt(modalOrder?.order?.type_order_) == 1) {
      ...
      if (
        parseInt(modalOrder?.order?.status_order) >= 1 &&
        parseInt(modalOrder?.order?.status_order) <= 3
      ) {
      ...
      <div className="header_addr_table">
        ...
        <div>
          {parseInt(modalOrder?.order?.type_order_) == 1 ? (
            ...
            <PicupModalOrderIcon fill="#DD1A32" />
          )}
          {isShowAddr ? (
          ...
          {parseInt(modalOrder?.order?.type_order_) == 1 ? (
            isShowAddr ? (
              <EyeShow_modalOrder fill="black" />
            ) : (
              <EyeHide_modalOrder />
            )
          ) : (
            <EyeShow_modalOrder fill="white" />
          )}
        </div>
      </div>
