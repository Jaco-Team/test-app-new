...
export const useHeaderStoreNew = reuseHotStore(
  'header',
          ...
          if (token && token.length > 0) {
            ...
            if (json?.st === false) {
              ...
              set({
                ...
                shortName: json?.user?.name
                  ? json?.user?.name?.substring(0, 1).toUpperCase()
                  : '',
                isAuth: 'auth',
              });
...
export const useCartStore = reuseHotStore(
  'cart',
  createWithEqualityFn(
    (set, get) => ({
      items: [],
      ...
      setCartLocalStorage: (opts = { touch: true }) => {
        ...
        const updatedAt = opts?.touch
          ? Date.now()
          : prevUpdatedAt || Date.now();
      ...
      getInfoPromo: async (promoName, city) => {
        if (promoName?.length == 0) {
          ...
          const shouldPersistPromoName =
            Boolean(json) && json?.status_promo !== false;
      ...
      changeAllItems: () => {
        ...
        const allPriceWithoutPromo = items.reduce(
          (all, it) => all + it.count * it.one_price,
          0
        );
        ...
        if (promoInfo) {
          if (
            Cookies.get('promo_name') &&
            Cookies.get('promo_name').length > 0
          ) {
        ...
        if (typeof window !== 'undefined') {
          ...
          if (token1 && token1?.length > 0) {
            ...
            if (json && json?.length > 0) {
              if (address) {
                const findAddr = json?.find(
                  (addr) =>
                    addr.street === address.street && addr.home === address.home
                );
            ...
            if (get().typePay.id == 'online') {
              ...
              if (!YooMoneyCheckoutWidget) {
                ...
                Sentry.captureMessage(
                  'YooMoney checkout widget constructor is unavailable',
                  {
                    level: 'error',
                    tags: {
      ...
      getItems: () => {
        ...
        let itemsOffDops = allItems?.filter(
          (item) =>
            (parseInt(item.cat_id) !== 7 || item.disabled) &&
      ...
      check_need_dops: () => {
        ...
        my_cart?.forEach((el) => {
          ...
          if (parseInt(this_item['cat_id']) == 14) {
            ...
            if (
              parseInt(this_item['cat_id']) !== 14 &&
              parseInt(this_item['cat_id']) !== 5 &&
              parseInt(this_item['cat_id']) !== 6 &&
              parseInt(this_item['cat_id']) !== 7
            ) {
      ...
      check_max_count: (item_id) => {
        ...
        let check_item = all_items?.find(
          (item) => parseInt(item.id) == parseInt(item_id)
        );

        if (
          parseInt(check_item.id) == 231 ||
          parseInt(check_item.id) == 232 ||
          parseInt(check_item.id) == 233 ||
          parseInt(check_item.id) == 351
        ) {
          return 1;
        }

        if (
          parseInt(check_item.type) != 3 ||
          (parseInt(check_item.id) !== 17 && parseInt(check_item.id) !== 237)
        ) {
          return 99;
        }
        ...
        my_cart?.forEach((item_cart, key) => {
          let item_info = all_items?.find(
            (item) => parseInt(item.id) == parseInt(item_cart['item_id'])
          );
          let check_free = free_items?.find(
            (item) =>
              parseInt(item['item_id']) == parseInt(item_cart['item_id'])
          );

          if (
            check_free &&
            check_free.max_count &&
            parseInt(item_info.type) != 3
          ) {
            ...
            if (parseInt(unic_item) == parseInt(item_free['dop_item_id'])) {
              ...
              new_free_dop?.forEach((el, k) => {
                if (parseInt(el['item_id']) == parseInt(unic_item)) {
                  check = true;
                  new_free_dop[k]['count'] +=
                    item_free['count_in_cart'] * item_free['max_count'];
                }
              });
        ...
        if (new_free_dop?.length > 0) {
          let max_count2 = new_free_dop?.find(
            (item) =>
              parseInt(item['item_id']) == 17 ||
              parseInt(item['item_id']) == 237
          );
          max_count = new_free_dop?.find(
            (item) => parseInt(item['item_id']) == parseInt(item_id)
          );
      ...
      plus: (item_id) => {
        ...
        if (isUtensil(id)) {
          ...
          if (!hasMainDish) {
            useHeaderStoreNew
              ...
              .setActiveModalAlert(
                true,
                'Приборы положены только к блюдам',
                false
              );
            return;
          }
        ...
        items = items.map((item) => {
          ...
          if (
            ...
            parseInt(item.count) + 1 <= max_count
          ) {
            item.count++;
            itemsCount++;
            check = true;
            ...
            useProfileStore
              .getState()
              .saveUserActions('plus_item', item.name, item.one_price, item_id);
        ...
        if (!found) {
          ...
          if (item) {
            ...
            useProfileStore
              .getState()
              .saveUserActions('plus_item', item.name, item.price, item_id);
          }
        ...
        const allPriceWithoutPromo = items.reduce(
          (all, it) =>
            parseInt(all) + parseInt(it.count) * parseInt(it.one_price),
          0
        );
      ...
      minus: (item_id, type) => {
        ...
        let check_dop = items?.filter(
          (item, key) =>
            parseInt(item.count) > 0 &&
            (parseInt(item.item_id) == 17 || parseInt(item.item_id) == 237)
        );
        ...
        items.map((item, key) => {
          ...
          if (parseInt(max_count) < 0) {
            items[key]['count'] =
              parseInt(item.count) + parseInt(max_count) >= 0
                ? parseInt(item.count) + parseInt(max_count)
                : 0;
          }
        });
      ...
      promoCheck: () => {
        ...
        let cart_new_promo = [];
        allPrice = my_cart.reduce(
          (sum, item) => sum + parseInt(item['all_price']),
          tmp
        );
        ...
        type_order = get().typeOrder == 'pic' ? 1 : 0;
        point_id_dev = get().orderAddr
          ? parseInt(get().orderAddr?.point_id)
          : 0;
        ...
        if (promo_info) {
          ...
          if (promo_info.limits.date.min && promo_info.limits.date.max) {
            if (
              this_date >= promo_info.limits.date.min &&
              this_date <= promo_info.limits.date.max
            ) {
          ...
          if (
            promo_info.limits.time.min != 0 &&
            promo_info.limits.time.max != 0
          ) {
            if (
              this_time >= promo_info.limits.time.min &&
              this_time <= promo_info.limits.time.max
            ) {
          ...
          if (
            promo_info.limits.summ.min != 0 ||
            promo_info.limits.summ.max != 0
          ) {
            if (
              allPrice >= promo_info.limits.summ.min &&
              (promo_info.limits.summ.max >= allPrice ||
                promo_info.limits.summ.max == 0)
            ) {
            } else {
          ...
          if (promo_info.limits.items.length > 0) {
            ...
            promo_info.limits.items.map((need_item) => {
              this_item = new_my_cart?.find(
                (item) => item.item_id == need_item
              );
          ...
          if (
            check_free_drive &&
            check_free_drive.length > 0 &&
            check_free_drive == '1722474061' &&
          ...
          if (parseInt(promo_info.promo_action) == 1) {
            ...
            if (parseInt(promo_info.sale.cat_sale) == 1) {
              ...
              my_cart?.forEach((el_cart, key_cart) => {
                this_item = allItems?.find(
                  (item) => item.id == el_cart.item_id
                );

                if (
                  parseInt(this_item.type) != 3 &&
                  parseInt(this_item.type) != 4
                ) {
                  promo_info.sale.sale_action?.forEach((el_promo) => {
                    if (parseInt(el_cart.item_id) == parseInt(el_promo)) {
                      if (parseInt(promo_info.sale.type_price) == 1) {
                        ...
                        if (count_sale > 0) {
                          all_price =
                            parseInt(el_cart.one_price) *
                              parseInt(el_cart.count) -
                            parseInt(count_sale);
                          ...
                          my_cart[key_cart].new_one_price = parseInt(
                            el_cart.one_price
                          );
                        ...
                        all_price =
                          ...
                          (parseInt(el_cart.all_price) / 100) *
                            parseInt(count_sale);
              ...
              my_cart?.forEach((el_cart, key_cart) => {
                ...
                if (
                  ...
                  if (parseInt(promo_info.sale.type_price) == 1) {
                    ...
                    if (count_sale > 0) {
                      all_price =
                        parseInt(el_cart.one_price) * parseInt(el_cart.count) -
                        parseInt(count_sale);
            ...
            if (parseInt(promo_info.sale.cat_sale) === 444) {
              ...
              my_cart.forEach((el_cart, idx) => {
                const this_item = allItems?.find(
                  (it) => it.id == el_cart.item_id
                );
            ...
            if (parseInt(promo_info.sale.cat_sale) == 7) {
              ...
              my_cart?.forEach((el_cart, key_cart) => {
                ...
                if (
                  parseInt(el_cart.all_price) > 0 &&
                  parseInt(el_cart.count) > 0
                ) {
                  if (parseInt(promo_info.sale.type_price) == 1) {
                    //рубли
          ...
          if (parseInt(promo_info.promo_action) == 2) {
            promo_info?.items_add?.forEach((el) => {
              this_item = allItems?.find(
                (item) => parseInt(item.id) == parseInt(el.item_id)
              );
            ...
            allPrice += cart_new_promo.reduce(
              (sum, item) => sum + parseInt(item['all_price']),
              tmp
            );
                ...
                promo_info?.items_on_price?.forEach((el_promo) => {
                  if (parseInt(el_cart.item_id) == parseInt(el_promo.id)) {
                    ...
                    my_cart[key_cart].all_price =
                      parseInt(el_promo.price) * parseInt(el_cart.count);
                  }
                });
              });
      ...
      repeatOrder: (order) => {
        ...
        set({
          ...
          orderPic:
            parseInt(order?.order?.type_order_) == 2
              ? order?.order?.pic_info
              : null,
        ...
        const allPriceWithoutPromo = my_cart.reduce(
          (all, it) =>
            parseInt(all) + parseInt(it.count) * parseInt(it.one_price),
          0
        );
        const itemsCount = my_cart.reduce(
          (all, it) => parseInt(all) + parseInt(it.count),
          0
        );
...
export const useContactStore = reuseHotStore(
  'contact',
  createWithEqualityFn(
    (set, get) => ({
      myAddr: [],
        ...
        if (point) {
          ...
          if (!firstPoint?.addr) {
            captureMapStateIssue(
              'Contacts map has no default point to select',
              {
...
export const useAkciiStore = reuseHotStore(
  'akcii',
  createWithEqualityFn(
    (set) => ({
      actii: [],
          ...
          if (new_banner?.info && Object.keys(new_banner?.info).length > 0) {
            ...
            if (parseInt(new_banner.info.promo_action) == 2) {
              new_banner.info.items_add.map((item, key) => {
                let find_item = all_items?.find(
                  (f_item) => parseInt(f_item.id) == parseInt(item.item_id)
                );
            ...
            if (parseInt(new_banner.info.promo_action) == 3) {
              new_banner.info.items_on_price.map((item, key) => {
                new_banner.info.items_on_price[key]['img_app'] =
                  all_items?.find(
                    (f_item) => parseInt(f_item.id) == parseInt(item.id)
                  )['img_app'];
              });
...
const resolveOrderIds = (get) => {
  ...
  return {
    order_id:
      st?.dataOrder?.order_id ?? st?.modalOrder?.order?.order_id ?? null,
    point_id:
      st?.dataOrder?.point_id ?? st?.modalOrder?.order?.point_id ?? null,
  };
};

export const useProfileStore = reuseHotStore(
  'profile',
  createWithEqualityFn(
    persist(
      (set, get) => ({
        ...
        setActiveAddressModal: async (active, id, city = '') => {
          if (active) {
            if (useHeaderStoreNew.getState().city !== city) {
              const findCity = useCitiesStore
                .getState()
                .thisCityList.find((item) => item.link === city);
        ...
        orderDel: async (this_module, userToken, text) => {
          ...
          if (!order_id || !point_id) {
            useHeaderStoreNew
              ...
              .setActiveModalAlert(
                true,
                'Откройте заказ заново и попробуйте отменить ещё раз',
                false
              );
            return;
          }
        ...
        saveUserActions: async (event, param, price = 0, item_id = 0) => {
          ...
          const token_tmp =
            (await useHeaderStoreNew.getState().token_tmp) ?? '';
...
export const useFooterStore = reuseHotStore(
  'footer',
  createWithEqualityFn(
    (set, get) => ({
      links: {},
...
export const useCitiesStore = reuseHotStore(
  'cities',
  createWithEqualityFn(
    (set) => ({
      thisCity: null,
...
export const useHomeStore = reuseHotStore(
  'home',
  createWithEqualityFn(
    (set, get) => ({
      bannerList: [],
      ...
      filterItems: (res, skipScroll = false) => {
        ...
        if (parseInt(res) !== parseInt(tag_filter)) {
          ...
          let find_tag = all_tags?.find(
            (tag) => parseInt(tag.id) === parseInt(res)
          );
      ...
      scrollToTargetAdjusted: () => {
        ...
        if (matches) {
          ...
          const banner = document
            .querySelector('.BannerMobile')
            ?.getBoundingClientRect().height;

          const menu =
            document.querySelector('.menuCatMobile')?.getBoundingClientRect()
              .height * 0.75;
          ...
          const header =
            document.querySelector('.headerNew')?.getBoundingClientRect()
              .height / 2;

          const banner = document
            .querySelector('.BannerPC')
            ?.getBoundingClientRect().height;
      ...
      getOneBanner: async (this_module, city, link) => {
        ...
        if (!json?.banner) {
          ...
          useProfileStore
            .getState()
            .saveUserActions('open_page_banner', json?.banner?.name);
        ...
        itemsCatInFlightPromise = api(this_module, data)
          .then((json) => {
            if (
              ...
              if (
                Array.isArray(stateWithFallback?.CatsItems) &&
                stateWithFallback?.CatsItems.length > 0
              ) {
                return {
      ...
      seedItemsCatFromPage: (cats = [], allItems = [], city = '') => {
        ...
        const currentCatsItems = Array.isArray(get().CatsItems)
          ? get().CatsItems
          : [];
        ...
        const hasNestedCategories = normalizedCategory.some(
          (cat) => Array.isArray(cat?.cats) && cat.cats.length > 0
        );
        ...
        for (const [catId, catItems] of itemsByCategory.entries()) {
          ...
          leafById.set(catId, {
            ...
            main_id: String(
              currentMeta?.main_id ?? currentMeta?.parent_id ?? catId
            ),
        ...
        if (
          currentCatsItems.length > fallbackCatsItems.length &&
          currentCatsItems.length > 0
        ) {
          return {
            category: get().category,
      ...
      getItem: async (this_module, city, item_id, type) => {
        ...
        useProfileStore
          .getState()
          .saveUserActions('open_item_home', json?.name, json?.price, item_id);
      ...
      closeTypeModal_dop: (event) => {
        if (
          event.target.classList.contains('first_text') ||
      ...
      setActiveBanner: (active, banner, swiper) => {
        ...
        if (banner) {
          ...
          useProfileStore
            .getState()
            .saveUserActions('open_banner_home', new_banner?.name);
