...
Конструктор может вернуть `data.conditions` со звёздами, товарами или категориями. Сайт поддерживает `conditions.stars`, `conditions.products` и `conditions.categories` и объединяет их с `data.showWhen`: элемент показывается, только если выполнены все заданные условия. До выбора рейтинга условные группы причин скрыты.
...
## Активная анкета сайта v1

В рабочей базе созданы две активные формы владельца `user_id = 1`. Обе назначены только каналу `client_site` с приоритетом `500`; форма №24 и канал `quality_control` не изменены.

| `form_id` | Название                       | Назначение                                                         |
| --------- | ------------------------------ | ------------------------------------------------------------------ |
| `25`      | Сайт — общая оценка и блюда v1 | Общая оценка, причины, комментарий и детализация по каждому товару |
| `26`      | Сайт — получение заказа v1     | Доставка, самовывоз, обслуживание в зале и заказ с собой           |

Форма №25 содержит обязательный на сайте общий `rating`. При оценке `4–5` показывается «Что особенно понравилось?», при `1–3` — «Что нам улучшить?». Комментарий `textarea` необязателен. В карточке каждого блюда доступны необязательные рейтинг и условные причины. Кнопка отправки активируется сразу после общей оценки: переходить в блюда или получение заказа не обязательно.

Форма №26 содержит необязательный рейтинг получения заказа и причины. Для `type_order = 1` используется доставка, для `2` — самовывоз, для `3` — обслуживание в зале, для `4` — заказ с собой. Курьерская иллюстрация отображается только для доставки.

Созданы 36 preset-param:

- `site_order_{taste|temperature|appearance|portion|packaging|accuracy}_{positive|negative}`;
- `site_product_{taste|temperature|appearance|portion}_{positive|negative}`;
- `site_delivery_{speed|courier|care|contact}_{positive|negative}`;
- `site_service_{speed|employee|readiness|convenience}_{positive|negative}`.

Условные группы одновременно содержат `conditions.stars` и совместимый `showWhen`. Положительные причины разрешены для `4–5`, отрицательные — для `1–3`. При смене рейтинга ответы из скрывшейся группы не попадают в `responses`; значения `checkboxGroup` дедуплицируются по `param`.

Mock повторяет формы №25 и №26. Для отката достаточно выключить эти две формы в конструкторе. Формы, presets и уже сохранённые ответы удалять не нужно.
