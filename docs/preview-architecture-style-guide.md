...
Коммуникационное правило для агентной работы: в чат отправляются только короткие отчеты. Не вставлять diff'ы, patch output, тела файлов, before/after snippets и длинные кодовые блоки, если пользователь явно не попросил показать код.
