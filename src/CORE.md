...
- `widgets/layout/` — общий каркас страницы: `PageLayout`, `PageFooter`.
...
4. `PageLayout` монтирует `StoreBootstrap`, `AppShell`, `ConnectedHeader`, body страницы и `PageFooter`.
5. `StoreBootstrap` гидрирует Zustand-store из SSR-данных, затем запускает client-only шаги: session hydration, refresh баннеров/каталога, пересчет корзины, telemetry.
6. UI страницы читает данные либо из SSR view model, либо из client store, либо из обоих источников одновременно.
