...
function sanitizePathname(pathname) {
  ...
  for (let i = 1; i < parts.length; i++) {
    ...
    try {
      seg = seg.replace(/\p{Extended_Pictographic}+/gu, '');
    } catch {}
...
export const config = {
  ...
  matcher: [
    '/((?!monitoring|_next|favicon.ico|robots.txt|sitemap.xml|images|fonts|static).*)',
  ],
};
...
export async function proxy(request) {
  ...
  // Собираем финальный URL сразу: https + lower-case + чистка "мусорных" query
  ...
  // 2) Корень остаётся самостоятельной индексируемой страницей выбора города.
  ...
  if (
    target.searchParams.has('text') ||
    target.searchParams.has('showItem') ||
    ...
    for (const k of Array.from(target.searchParams.keys()))
      if (k.startsWith('act_')) target.searchParams.delete(k);
    changed = true;
  }
  ...
  response.cookies.set('sid_ts', String(now), {
    path: '/',
    maxAge: 60 * 30,
    httpOnly: true,
    sameSite: 'Lax',
